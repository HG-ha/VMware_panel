# VMware 16 Pro 个人计算机虚拟化管理面板（未完成）
...
### 运行该项目
1. 首先配置vmrest，需要确保您的vmrest是可用的，本项目不会去自动打开vmrest
2. 以管理员方式运行vmrest（否则没有权限进行虚拟网卡操作）
3. 运行gateway.py脚本，该脚本用于转发前端的请求并校验登录
4. 直接在浏览器打开template/login.index页面（纯浏览器解析，原生实现）
5. 开始使用面板吧
